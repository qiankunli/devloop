...
class Component:
    ...
    一个 git 仓可含多个 component（`server/` + `cli/`、`packages/*`、`cmd/*`）；「操作落在哪个
    component」由**操作目标路径**决定，不由 repo 的单值属性决定——这正是单值 `code_dir` 在多代码
    ...
    - `id`：仓相对路径（`.` / `server` / `cli`）= **持久化身份**，跨 checkout 稳定。落
      ...
      component 在 worktree 与主 checkout 下拿到不同 key——而 validation 段统一落**主仓**
      `branches/<b>/`，于是 worktree 里 lint 过的戳回主 checkout 就查不到（白跑一遍），且 key
      随 worktree 增删无限累积。

    `id` **必填、且只由 `at()` 推**：给它默认值的话，生产路径漏传就是个静默的空 key（多个 component
    撞进同一个戳），比不设身份更糟；而让消费方自己算 id 就要各自再传一次 git_root，root 传错
    （worktree 里传主仓根）算出的 key 是错的——绑在出生点上，这两类错都不可表达。

    component 也拥有**自己的工具链动作**（`has_target` / `lint_target` / `test_target`）：一个
    component「能不能 / 该怎么 lint/test」是它自己的事实，收在这里，消费方（checks / gate rules）
    直接问 component，而不是各自拿 `str` 路径去重解析 Makefile（避免 str 路径当隐式协议、同一判断
    出现多份实现）。"""
    path: str
    id: str
    ...
    def at(cls, path: str | Path, git_root: str | Path) -> "Component":
        """在 `git_root` 这个 checkout 里、位于 `path` 的 component——**唯一**的构造入口（生产侧）。
        ...
        try:
            ...
            cid = p.as_posix()
        return cls(str(path), cid, ecosystem.detect_language(path))
...
def find_repo_code_dir(repo_dir: str | Path) -> str:
    ...
    for sub in ("server", "backend"):
        ...
        if candidate.is_dir() and _is_component(candidate):
...
def owning_component(target: str | Path, git_root: str | Path) -> Component | None:
    """`target`（文件或目录）**属于**哪个 component——从它向上找最近的项目清单目录
    （`_is_component`），边界止于 `git_root`。**没有任何 component 拥有它就是 `None`**。

    `None` 是个**答案**，不是失败：仓根的 `README.md` / `docs/` / `.github/` 在「根不是 component」
    的仓（`server/` + `cli/`，doctor 就是）里，确实不落在任何 component 的项目边界内。硬给它派一个
    component 就是编造归属——而唯一能派的 `default_component` 是**选择**启发式（`server/` > `backend/`
    > 根），它答的是「没有具体目标时该选谁」，不是「这个文件属于谁」。让它来答归属，得到的是
    「改 README → 跑 server 的 lint」这种没有任何理由的结论（为什么是 server 不是 cli？）。

    走到仓根时**根自己是 component 就是它**（判据同子目录、同 `discover_components`）：catalog 说根
    是 component，归属就不能把根的文件判给别人，否则同一个文件走 clean-tree 全量和走改动投影会得到
    两个不同的 component。
    """
    root = Path(git_root).resolve()
    ...
    while cur != root and root in cur.parents:
        if _is_component(cur):
            return Component.at(cur, git_root)
        cur = cur.parent
    return Component.at(root, git_root) if _is_component(root) else None


def enclosing_component(target: str | Path, git_root: str | Path) -> Component:
    """站在 `target` 时**操作落在**哪个 component——必给一个答案：有 owner 就是它，没有则回落
    `default_component` 的选择启发式（`server/` > `backend/` > 根）。

    与 `owning_component` 的差别是**问的问题不同**，别混：

    - **归属**（`owning_component`，「这个文件属于谁」）：README 不属于任何 component，这是事实，
      答 `None` 才对。改动投影 / 指纹用它——凭空派个 component 会让纯文档改动去跑那个 component 的 lint。
    - **站位**（本函数，「我在这儿干活，算哪个 component」）：站在仓根跑 `pip install`，总得有个 component
      来判它是不是 uv 仓；`run_tests.py doctor` 走默认 `server/` 也是这一支。这时「没有具体目标
      该选谁」**正是**要问的问题，default 的启发式是对的问题的对的答案。

    命令侧 guard（`pip_install` / `pytest_naked`）与 `select_components(explicit=…)` 用本函数。"""
    return owning_component(target, git_root) or default_component(git_root)


# repo-wide 验证（clean tree 从仓根发起、无具体改动可依据）枚举全部 component 时跳过的目录：
# VCS / 依赖 / 虚拟环境 / 构建产物 / 各类缓存——不是本仓的项目，进去只会拖慢扫描。
# node_modules / vendor 尤其要跳：里面每个包都带 package.json / go.mod，不跳会扫出成百上千个「component」。
_DISCOVER_SKIP = {
...
def discover_components(git_root: str | Path, *, max_depth: int = 4) -> list[Component]:
    """枚举 repo 内**全部**独立 component（带语言项目清单的目录，见 `_is_component`），供
    repo-wide 验证用。

    遇到一个 component 就收下、**不再下钻它内部**（component 内的嵌套清单不算独立 component）；跳过 VCS /
    依赖 / 构建产物目录，并限制递归深度。多代码目录仓（`server/` + `cli/`）因此返回全部 component。

    **仓根不是特例**：判据与子目录同一条，所以「根是 Go module + `server/` 是 Python component」的仓
    会同时收到两者。这里刻意**不问 `default_component`**——那是「没有更具体
    ...
    **目录事实**问题，答案会被 `server/` 的存在改写：`server/` 在时它返回 `server/`，而 `server/`
    ...
    绝不退回单值默认 component——所以这里返回「全部」而非「某一个」。"""
    root = Path(git_root).resolve()
    ...
    def walk(d: Path, depth: int) -> None:
        ...
        for child in children:
            # component 只属于当前 repo：不跟随仓内软链（可能自指/指向仓外），
            ...
            if _is_component(child):
                components.append(Component.at(child, root))
                # 命中即止：不下钻，component 内部的子 marker（如 packages 内嵌）不当独立 component
            else:
                walk(child, depth + 1)

    # 根自身是 component 就收下，与子目录同一判据。walk 只看子目录、从不收根，故无需去重。
    if _is_component(root):
        components.append(Component.at(root, root))
    walk(root, 1)
    if not components:
        # 全仓一份项目清单都没有：仍给出唯一 component（探测出的 server/backend/根），绝不返回空——
        # 空会被消费方读成「没什么要验的」而静默跳过。这里才轮到选择启发式登场。
        components.append(default_component(root))
    return components


def _is_component(path: Path) -> bool:
    """`path` 自身是不是一个 component —— 它带没带某个语言的**项目清单**。

    **刻意不看 `Makefile`**：Makefile 是 component 的**动作入口**（怎么 lint/test，见 `lint_target`），
    不是它的**身份**。两者正交——doctor 的 `server/`(pyproject) 和 `cli/`(package.json) 都有
    Makefile，但让它们成为 component 的是前者。反过来，`docs/` 里放个 sphinx 的 Makefile 不该让 docs
    变成 component，仓根的编排 Makefile（`make dev` 转发到子目录）也不该让仓根变成 component。

    **也不看 `requirements.txt`**：那是依赖清单不是项目边界（常见形态恰恰是仓根放一份给容器构建、
    真项目在 `server/`——认它就等于把仓根误判成 component，正是这里要消灭的那类误判）。它仍是
