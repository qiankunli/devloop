...
"which repo?" argument. `repo_resolve`'s own error messages already prescribe `--repo`
...
from . import repo_resolve

# Single source of truth for the help text, deliberately worded like repo_resolve's failure
...
def add_repo_arg(parser: argparse.ArgumentParser, *, positional: bool = True) -> None:
    ...
    `--repo/-r` is the canonical form (the one repo_resolve's errors tell callers to use).
...
) -> tuple[repo_resolve.ResolvedRepo, str]:
    """Resolve the repo-target arg to a ResolvedRepo, or print the reason and exit(1).

    The single failure path for repo resolution, so every script reports it identically
    (prefixed with `prog`). Returns (ResolvedRepo, how) on success.
    """
    resolved, how = repo_resolve.resolve_repo_dir(repo_target(ns), cwd)
    if not resolved:
