...
adapters that consume them live elsewhere (e.g. `hooks/cmdtree/parable.py`).
...
- **Used by**: `hooks/cmdtree/parable.py` (the Parable→`cmdtree.base` backend), behind the
  swappable parser seam in `hooks/cmdtree/cmdparse.py`.
