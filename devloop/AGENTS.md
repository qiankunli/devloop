...
- **三域布局（worktree 感知）**：`.devloop/` 按状态描述的对象分三域——**repo 域**（meta/pr/remote_branches/requirements/friction 等，落**主仓** `.devloop`，linked worktree 经 `.git` 文件解析归位，worktree 清理不丢数据）；**branch 域**（branch/validation/injection/review，落主仓 `branches/<branch>/`，load 按 **live 分支**取段——"陈旧分支缓存愚弄展示"一族问题结构性消失；git 禁止同分支双检出 → 每文件单写者天然成立）；**working-tree 域**（owner 锁，留各 worktree 自己的 `.devloop`——并行 worktree 不被误串行化）。submodule 形态回落本地，绝不写宿主 `.git/modules`。解析原语在 `context/store.py`（`_main_repo_root` / `state_dir` / `worktree_state_dir` / `branch_segment` / `tmp_dir`——ephemera 归 `.devloop/tmp/`）。
