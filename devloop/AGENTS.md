...
- **分段状态总线（为什么 repo 级不是单文件）**：repo 级状态按 writer-owner 拆成段文件、`load` 合并成视图——写入角色分散在不同进程，单文件逼出"读-改-写"、并发丢更新；一段一 owner 让每次写只碰不相交的文件，**多写者丢更新在结构上不可能、无需锁**。原子写 / fail-open 读与三域解析的原语在 `context/store.py`（`base.py` 只剩共享词汇：叶子/常量/时间）。
- **三域布局（worktree 感知）**：`.devloop/` 按状态描述的对象分三域——**repo 域**（meta/pr/remote_branches/requirements/friction 等，落**主仓** `.devloop`，linked worktree 经 `.git` 文件解析归位，worktree 清理不丢数据）；**branch 域**（branch/lint/test/injection/review，落主仓 `branches/<branch>/`，load 按 **live 分支**取段——"陈旧分支缓存愚弄展示"一族问题结构性消失；git 禁止同分支双检出，故**跨 checkout** 的单写者天然成立——但**进程内**并发不在这条轴上：`dispatch` 并发跑的 lint 与 test 曾共用一个 `validation.json`，实测互相覆盖丢戳，故按 writer-role 各占一段）；**working-tree 域**（owner 锁，留各 worktree 自己的 `.devloop`——并行 worktree 不被误串行化）。submodule 形态回落本地，绝不写宿主 `.git/modules`。解析原语在 `context/store.py`（`_main_repo_root` / `state_dir` / `worktree_state_dir` / `branch_segment` / `tmp_dir`——ephemera 归 `.devloop/tmp/`）。
