...
后台 run_review 退出后会唤醒会话。agent 这一轮：
...
3. 按 High / Medium 分组汇报；`start_line==end_line==0` 表示定位失败，读该文件按 content 定位。
4. **修复要 gate**：用户明确要"review 并修"或处于 auto-accept 类 mode → 改 High/Medium；
   否则只摆建议等确认。**review 是 advisory，从不代替人 merge。**
