...
| 3' | 开发后效 | `PostToolUse` | `posttool_track_edits` / `posttool_git_refresh` | 编辑计数按被改文件所属 code unit 累计；git 状态命令后按**位置感知的有效目录**刷新对应 repo 的 branch 段 |
| 4 | 验证 | `/lint` `/test` | `run_fixlint` / `run_tests` → `lifecycle.checks` | 按 code unit 盖 validation 戳（该 unit 的 lint/test 时间 + 该 unit 编辑计数清零） |
