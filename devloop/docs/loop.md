...
```
enter 某子模块 → 提需求 → 开发 → 验证(lint/test) → commit/建 MR → 人工 merge → 下一轮
```
...
| 2 | 每轮对话 | `UserPromptSubmit` | `userprompt_inject` | turn 注入（branch / dirty / validation / MR 摘要，内容哈希 dedup）；`PostCompact` 清 dedup 强制重发；AGENTS.md 被改时 `FileChanged` → `filechanged_refs` 重注入 References |
...
| 3' | 开发后效 | `PostToolUse` | `posttool_track_edits` / `posttool_git_refresh` | 编辑计数累计；git 状态命令后按**位置感知的有效目录**刷新对应 repo 的 branch 段 |
| 4 | 验证 | `/lint` `/test` | `run_fixlint` / `run_tests` → `mark_validation` | validation 戳（lint/test 时间 + 编辑计数清零） |
| 5 | 提交 / MR | `/gcam` `/gcamp` `/gcampr` | `smart_git_ops`（PLAN banner 自陈） | 新分支 cut 自 `origin/<target>`（base 由意图定）；`--files` 收敛 staging；外来提交自检；建 MR 后触发一次 `poll_mr_status` |
| 6 | 等人工 merge | monitors 周期轮询 | `poll_mr_status` | 写 `mr.json` 窗口 → 当前分支派生为 in-flight，turn 注入软提示 |
| 7 | merge 后下一轮 | （人工，AI 范围外） | — | 分支派生为 inactive，编辑被硬拦；回到第 1 拍，从最新 `origin/<target>` 切新分支 |

### 分支一轮的生命线

```
cut off origin/<target> → healthy ──push+MR──▶ in-flight ──人工 merge(AI 范围外)──▶ inactive
         ▲                                                                            │
...
- **分支四态**（拍 2 的提示内容、拍 3 / 5 的拦截依据）：protected / healthy / in-flight / inactive，全部由 `mr.json` 窗口派生、不存 bool。见 CONCEPTS.md〈分支状态流转〉。
...
- **为什么循环里到处是"派生不存 bool"**：拍 6/7 的状态翻转发生在 AI 范围外（人工 merge），任何落盘的 bool 都会过期；按 `mr.json` 窗口现算，切分支即自动失效、无人去清。
- **为什么 guard 全部 fail-open**：循环的主产出是开发本身，guard 是护栏不是闸门——任何 guard 内部错误都不得打断用户的工具调用（`hook_io` runner 保证）。
