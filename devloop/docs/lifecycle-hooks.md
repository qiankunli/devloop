...
| `run_lifecycle_gate` / `phase_paths` | `scripts/smart_git_ops.py` | 在 commit/mr 流水线里的 dispatch 插点 + 算出本相位「本次改动」的范围 |
| `select_units` / `committed_paths` / `range_paths` | `hooks/lib/repo_resolve.py` | 范围 → code unit 投影；各相位取「本次改动」的 git 问法 |
...
所以范围是 `dispatch(phase, repo, paths=...)` 的入参，由 `smart_git_ops.phase_paths` 在相位
...
`--files` 尤其要进 pre_commit 的范围：它是「工作树所有脏文件」的**子集**。拿超集当范围，会把你压根不打算提交的 unit 拖进 gate——它有存量 lint 错误就拦掉你的 commit（与本表存在的理由同一类失败，只是换了扇门），且 lint 的 `make fix` 会去改那些 unit、改完又不进本次 commit，凭空搅脏工作树。故 `--files` 在 `smart_git_ops.main` 归一**一次**（仓根相对），gate 与 staging 共用同一份——各自归一必然漂。
