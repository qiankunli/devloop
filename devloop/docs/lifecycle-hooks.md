...
`HookResult`（不在 subprocess 里跑 ocr），`run_lifecycle_gate` 把 `res.to_launch` emit 成
PLAN 的 `ARMED:` 行，agent 读到后用 `run_in_background` 起 `scripts/run_review.py`，跑完
harness 唤醒会话汇报。完整契约见 [`code-review.md`](./code-review.md)。
