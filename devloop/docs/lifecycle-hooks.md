...
| 对象 | 位置 | 职责 |
|---|---|---|
| `dispatch` / `HookResult` / `BackgroundSpec` | `lib/lifecycle/base.py` | facade：并发 join + 聚合；纯机制 |
| `lint` / `test` handler | `lib/lifecycle/checks.py` | 内置 inline gate handler（与 `/lint` `/test` 共用同一段逻辑） |
| `run_lifecycle_gate` / `phase_paths` | `scripts/commit_flow.py` | 在 commit/mr 流水线里的 dispatch 插点 + 算出本相位「本次改动」的范围 |
| `select_units` / `committed_paths` / `range_paths` | `lib/repo.py` | 范围 → code unit 投影；各相位取「本次改动」的 git 问法 |
...
一次 `/gcam`（或 gcamp / gcampr）的同步主链路（`commit_flow.main`）：
...
**两个执行点，一份策略。** 正常 commit 走 `/gcam`→commit_flow，那里 dispatch 真跑
...
`relay`（`BackgroundSpec`）返回，**由调用方 detach 起**：commit_flow 在 git 动作完成后
...
handler 手里只有 `repo`。想知道「本次改动涉及哪些 code unit」，它只能去读工作树——而**那个
答案只在 `pre_commit` 成立**：改动尚未提交，工作树就是答案。commit 之后工作树是干净的，同一
个问法会得到「什么都没改」，被 `select_units` 读成「不知道范围」，进而退化成 repo-wide 跑
**全部** unit。后果不是慢一点：一个你根本没碰、却有存量 lint 错误的 unit 会让 `post_commit` /
`pre_mr` 的 gate fail，而此时 commit 已经落地——push 和 MR 全被拦，改动本身却是好的。

所以范围是 `dispatch(phase, repo, paths=...)` 的入参，由 `commit_flow.phase_paths` 在相位
边界算好后**冻结**下传，每个相位问自己的那个 git 问题：
...
`--files` 尤其要进 pre_commit 的范围：它是「工作树所有脏文件」的**子集**。拿超集当范围，会把你压根不打算提交的 unit 拖进 gate——它有存量 lint 错误就拦掉你的 commit（与本表存在的理由同一类失败，只是换了扇门），且 lint 的 `make fix` 会去改那些 unit、改完又不进本次 commit，凭空搅脏工作树。故 `--files` 在 `commit_flow.main` 归一**一次**（仓根相对），gate 与 staging 共用同一份——各自归一必然漂。

**`None` 与 `[]` 是两种语义，不能抹平**：`None` = 不知道范围 → 全跑（保守）；`[]` = 知道且
为空 → 0 个 unit、干净跳过。git 算不出时（如 `origin/<target>` 尚未 fetch）一律返回 `None`
而非 `[]`——`gitcmd` 是 failure-safe 的，两者原始输出都是空，把失败读成 `[]` 就等于**静默跳过
整个 lint gate**。gate 上宁可多跑，不可 fail-open。
