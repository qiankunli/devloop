...
branch 域 `branches/<b>/validation.json`：验证戳**按 code unit 键**——key 是 unit 的仓相对身份 `CodeUnit.id`（`.` / `server` / `cli`），每个 unit 各记自己的 lint / test 通过时间，以及 **lint 通过那一刻的内容指纹**（`repo_resolve.unit_fingerprint`）。
