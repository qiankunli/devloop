...
branch 域 `branches/<b>/validation.json`：验证戳**按 code unit 键**——key 是 unit 的仓相对身份 `CodeUnit.id`（`.` / `server` / `cli`），每个 unit 各记自己的 lint / test 通过时间与距上次 lint 的编辑累计（PostToolUse Edit 按**被改文件所属 unit** 自增，该 unit lint 通过时清零）。
