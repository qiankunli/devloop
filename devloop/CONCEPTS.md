...
- **`code unit`**（`repo_layout.CodeUnit`）：仓库内一个自带工具链、可独立 `make` / `uv` build/lint/test 的目录，`make` / `uv` 的 workdir。一个 git 仓可含**多个** unit（`server/` + `cli/`、`packages/*`、`cmd/*`）——所以「操作落在哪个 unit」由**操作目标路径**决定（`enclosing_code_unit`：从目标向上找最近的工具链目录，止于仓根），不是 repo 的单值属性；这正是过去单值 `repo_code_dir` 在多代码目录仓上选错目录的根因。unit 在解析边界一次算清 path+language 随 `ResolvedRepo` 下传，消费方不再各自 `detect_language` 重推。
