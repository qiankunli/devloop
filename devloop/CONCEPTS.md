...
- **`component`**（`repo_layout.Component`）：仓库内一个可独立 build/lint/test 的项目目录，也是该 component 工具命令的 workdir。**身份由语言的项目清单定义**（`_is_component`：`go.mod` / `pyproject.toml` / `setup.py` / `package.json`——TS 与 JS 同为 `package.json`，`tsconfig.json` 只是编译配置、一个 package 里可有多份，不定义边界）。**`Makefile` 刻意不算身份**：它是 component 的**动作入口**（怎么 lint/test，见下）——doctor 的 `server/` 与 `cli/` 都有 Makefile，但让它们成为 component 的是 pyproject / package.json；反过来 `docs/` 里一个 sphinx Makefile、仓根一个转发用的编排 Makefile，都不该因此变成 component。`requirements.txt` 同理不算（依赖清单不是项目边界，常见形态正是仓根放一份给容器构建、真项目在 `server/`），但它仍是 `detect_language` 的语言线索——「这是什么语言」和「这是不是一个项目」是两个问题。一个 git 仓可含**多个** component（`server/` + `cli/`、`packages/*`、`cmd/*`），**仓根也可以是其中之一**（根是 Go module + `server/` 是 Python component 时两者都在）——所以「哪个 component」由**路径**决定、不是 repo 的单值属性。这里有**两个不同的问题**，别混：
  - **归属**（`owning_component`）：「这个**文件属于**谁」。从它向上找最近的项目清单目录，止于仓根；**没有 component 拥有它就是 `None`**——仓根的 `README.md` / `docs/` / `.github/` 在「根不是 component」的仓里确实不在任何 component 的项目边界内，`None` 是答案不是失败。改动投影（`select_components`）与 lint 指纹用它：共享路径**不贡献 component 也不减少**，于是纯文档改动不触发任何 component 的验证。刻意**不做**「共享路径→全部 component」——那是拿最常见的情况（改文档/CI）去付最罕见情况的账，每个文档 PR 都跑全仓、任一 component 有存量错就拦你。代价是根级 tooling 配置（`ruff.toml`）改动不触发验证：它在下一次任何 lint 里立刻暴露，且 CI 兜底。
  - **站位**（`enclosing_component`）：「我**在这儿干活**算哪个 component」。必给答案：有 owner 就是它，没有则回落 default 的选择启发式。命令侧 guard（站在仓根跑 `pip install`，总得有个 component 判 uv）与 `select_components(explicit=…)` 用它。
...
分支是否受保护是**派生**的（`Branch.is_protected()`，**不存进** `branch.json`）：注入头部据此打 `⚠️ PROTECTED`，而 `protect_branch` 硬 gate 走 `gate.evaluate()` 按 **live 分支名**判——缓存陈旧（观测不到的 checkout 后）也不会漏判保护分支。判定规则（`lib/git_state.py`）：`main` / `master` 严格匹配；`release` 严格匹配或以 `release` 开头 / 结尾；`release` 出现在中间不算（`feat/release-notes` 不受保护）。
...
- **component 不拆成目录**：component **不是 writer**（同一 check 内 fan-out 是顺序的），拆了不解决这个 race；且 component id 不是安全路径分量（根 component 是 `.`、可含斜杠如 `eval/reviewbench`），当目录还得枚举目录才能读回全集。当 JSON key 三个问题都没有。

**lint 通行证绑内容，不绑「改了几次」**：gate 拿当前指纹与戳上的比，不等 = 内容变过 = 通行证作废。指纹 hash 的是该 component 名下**已改动文件的当前 bytes**（删除写 tombstone、symlink 写 target），不是 diff——新建的未跟踪文件改了内容路径不变，diff 看不出来，而 lint 会 lint 它。算不出 → gate 按未验证处理（fail-closed）。
...
**为什么 key 是仓相对路径而非 `Component.path`**：后者是绝对路径（「这次在哪跑 make」），而 validation 段统一落**主仓** `branches/<b>/`——worktree 里的 `<repo>/.worktrees/foo/server` 与主 checkout 的 `<repo>/server` 是同一个 component，用绝对路径会让同一 component 在不同 checkout 下拿到不同 key（worktree 里 lint 过的戳回主 checkout 查不到，白跑一遍），且 key 随 worktree 增删无限累积。
...
PreToolUse 守卫统一成一个策略引擎（`hooks/core/` + `hooks/rules/`）：一次工具调用投影成 **`Change`**（携带 `Command` / `FileChange` 等 **`Target`**），跑匹配的 **`Rule`** 产出 `Finding`，聚合成 **`Decision`**（allow/warn/deny）。两个入口 hook——`pretool_policy_bash`（命令侧）、`pretool_policy_edit`（编辑侧）——把原先 11 个独立 guard 收成 2 个。
...
- **`Rule`**：可积累的判定单元，子类住 `hooks/rules/{command,edit,code}/`、登记进 `rules.REGISTRY`；引擎按 `target_kind` 路由。`needs_content` 的 FileChange 规则才触发内容解析。逐规则 **fail-open**（守卫的 bug 绝不拦用户）。命令侧规则按风险黑名单设计：默认放行，只拦高置信的项目级 / 协作级风险，不把未知命令当违规。
