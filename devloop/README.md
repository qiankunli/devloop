...
- **Board 与 PR/MR 感知**：workspace/repo 的 `.devloop/` 保存当前 branch、工作区、近期 PR/MR、验证和 session 归属等结构化运行态；hook 与 monitor 持续刷新事实，Board 按相关性选择紧凑内容并决定 session/turn 投递。在 tmux 中启动 CLI 时，插件还会自动显示固定三行 Board HUD：工作上下文、当前健康状态和最新变化。状态与投递游标都自动排除在 git 提交之外。
...
Board HUD 需要 CLI 已运行在 tmux 中；非 tmux 环境会安静跳过，不影响其余能力。
...
```jsonc
{
  ...
  // tmux 中默认自动显示三行 Board HUD；不需要时显式关闭。
